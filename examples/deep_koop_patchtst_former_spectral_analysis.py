...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 25 12:27:54 2025
...
Now extended with:
  ...
  - ρ_max ablation for full DeepKoopFormer variants (PatchTST, Autoformer, Informer)

2025-05-20  – Ali Forootani
Modified: spectral radius monitoring (2025-11-22)
Modified: ρ_max ablation for full DeepKoopFormer (2025-11-25)
"""
...
def compute_spectral_radius(koop_module: nn.Module | None) -> float:
    ...
    Compute spectral radius ρ(K) for:
      - StrictStableKoopmanOperator: K = U diag(Σ) V^T
      - LinearLatentPropagator:      K = W^T
    ...
    with torch.no_grad():
        if isinstance(koop_module, StrictStableKoopmanOperator):
            ...
            K = U @ torch.diag(Σ) @ V.T
        elif isinstance(koop_module, LinearLatentPropagator):
            K = koop_module.W.weight.T
        else:
            return 0.0
...
class Koopformer_PatchTST(nn.Module):
    ...
    """
    def __init__(self, input_dim, seq_len, horizon,
                 patch_len, d_model=64, rho_max: float = 0.99):
        super().__init__()
        self.backbone = PatchTST_Backbone(input_dim, seq_len,
                                          patch_len=patch_len,
                                          d_model=d_model)
        self.koop = StrictStableKoopmanOperator(d_model, ρ_max=rho_max)
        self.fc   = nn.Linear(d_model, horizon * input_dim)
        self.rho_max = rho_max  # for logging
...
class Koopformer_Autoformer(nn.Module):
    def __init__(self, input_dim, seq_len, horizon,
                 patch_len, d_model=16, rho_max: float = 0.99):
        super().__init__()
        self.backbone = SimpleAutoformer(seq_len, horizon, input_dim,
                                         patch_len=patch_len,
                                         d_model=d_model)
        # horizon-sized latent, ODO parameterisation
        self.koop = StrictStableKoopmanOperator(horizon, ρ_max=rho_max)
...
# --------------------------------------------------------------------------- #
# 15)  ρ_max ablation for full DeepKoopFormer variants                        #
# --------------------------------------------------------------------------- #
def rho_ablation_full_models(file="pressure_surface_2020.npy",
                             seq_len=120,
                             patch_len=120,
                             horizon=10,
                             indices=None,
                             rho_grid=None,
                             epochs=1000,
                             save_dir="results/rho_ablation"):
    """
    Ablation over ρ_max for the full DeepKoopFormer (ODO + Lyap) variants:
      - Koopformer_PatchTST
      - Koopformer_Autoformer
      - Koopformer_Informer

    For each ρ_max, trains the model, logs training spectral radius trajectory,
    saves Koopman matrix and spectrum, and writes metrics to CSV.

    This directly addresses the reviewer comment about approaching the unit
    circle and the stability–expressivity trade-off.
    """
    indices  = indices or [0, 1, 2, 3, 4, 5]
    rho_grid = rho_grid or [0.90, 0.95, 0.98, 0.995, 0.999]
    save_dir = Path(save_dir).expanduser().resolve()
    save_dir.mkdir(parents=True, exist_ok=True)

    metrics_file = save_dir / "metrics_rho_ablation.csv"
    if not metrics_file.exists():
        pd.DataFrame(columns=["Model", "RhoMax", "PatchLen",
                              "Horizon", "MSE", "MAE"]
                     ).to_csv(metrics_file, index=False)
    ...
    x_in, x_out, _ = build_dataset(raw, indices,
                                   seq_len, horizon)
    x_out_np = x_out.numpy()
    F, H = len(indices), horizon

    # For plotting spectral radii per model
    spec_histories = { "Koop-PatchTST-Full": {},
                       "Koop-Autoformer-Full": {},
                       "Koop-Informer-Full": {} }
    ...
    for rho_max in rho_grid:
        ...
        # 1) PatchTST backbone
        print("== Koopformer-PatchTST (FULL, ODO + Lyap) ==")
        koop_patch = Koopformer_PatchTST(F, seq_len, horizon,
                                         patch_len=patch_len,
                                         d_model=96,
                                         rho_max=rho_max)
        pred, loss, eig = train(koop_patch, x_in, x_out,
                                epochs=epochs,
                                koop_attr="koop",
                                lyap_weight=0.1,
                                spec_every=10)
        ...
        pd.DataFrame({
            "Model":   ["Koopformer_PatchTST_Full"],
            "RhoMax":  [rho_max],
            "PatchLen":[patch_len],
            "Horizon": [horizon],
            "MSE":     [mse],
        ...
        prefix = save_dir / f"KoopPatchTST_Full_rho{rho_max:.5f}_L{patch_len}_H{horizon}"
        np.save(f"{prefix}_rho_traj.npy", eig)
        K_np, evals_np, rho_final = compute_K_and_spectrum(koop_patch.koop)
        if K_np is not None:
            np.save(f"{prefix}_K.npy", K_np)
            np.save(f"{prefix}_eigvals.npy", evals_np)
        torch.save(koop_patch.cpu().state_dict(), f"{prefix}.pt")
        spec_histories["Koop-PatchTST-Full"][rho_max] = eig

        # 2) Autoformer backbone
        print("== Koopformer-Autoformer (FULL, ODO + Lyap) ==")
        koop_auto = Koopformer_Autoformer(F, seq_len, horizon,
                                          patch_len=patch_len,
                                          d_model=64,
                                          rho_max=rho_max)
        pred, loss, eig = train(koop_auto, x_in, x_out,
                                epochs=epochs,
                                koop_attr="koop",
                                lyap_weight=0.1,
                                spec_every=10)
        ...
        mae = float(np.abs(err).mean())
        pd.DataFrame({
            "Model":   ["Koopformer_Autoformer_Full"],
            "RhoMax":  [rho_max],
            "PatchLen":[patch_len],
            "Horizon": [horizon],
            "MSE":     [mse],
        ...
        prefix = save_dir / f"KoopAutoformer_Full_rho{rho_max:.5f}_L{patch_len}_H{horizon}"
        np.save(f"{prefix}_rho_traj.npy", eig)
        K_np, evals_np, rho_final = compute_K_and_spectrum(koop_auto.koop)
        if K_np is not None:
            np.save(f"{prefix}_K.npy", K_np)
            np.save(f"{prefix}_eigvals.npy", evals_np)
        torch.save(koop_auto.cpu().state_dict(), f"{prefix}.pt")
        spec_histories["Koop-Autoformer-Full"][rho_max] = eig

        # 3) Informer backbone
        print("== Koopformer-Informer (FULL, ODO + Lyap) ==")
        koop_inf = Koopformer_Informer(F, seq_len, horizon,
                                       patch_len=patch_len,
                                       d_model=96,
                                       rho_max=rho_max)
        pred, loss, eig = train(koop_inf, x_in, x_out,
                                epochs=epochs,
                                koop_attr="koop",
                                lyap_weight=0.1,
                                spec_every=10)
        ...
        mae = float(np.abs(err).mean())
        pd.DataFrame({
            "Model":   ["Koopformer_Informer_Full"],
            "RhoMax":  [rho_max],
            "PatchLen":[patch_len],
            "Horizon": [horizon],
            "MSE":     [mse],
        ...
        prefix = save_dir / f"KoopInformer_Full_rho{rho_max:.5f}_L{patch_len}_H{horizon}"
        np.save(f"{prefix}_rho_traj.npy", eig)
        K_np, evals_np, rho_final = compute_K_and_spectrum(koop_inf.koop)
        if K_np is not None:
            np.save(f"{prefix}_K.npy", K_np)
            np.save(f"{prefix}_eigvals.npy", evals_np)
        torch.save(koop_inf.cpu().state_dict(), f"{prefix}.pt")
        spec_histories["Koop-Informer-Full"][rho_max] = eig

    # Optional: summary plots of ρ(K) trajectories vs epochs
    for model_name, hist in spec_histories.items():
        fig, ax = plt.subplots(figsize=(6, 4))
        for i, (rho_max, traj) in enumerate(hist.items()):
            ax.plot(traj,
        ...
        ax.set_title(f"{model_name}: ρ(K) vs. epochs")
        ax.grid(True, linestyle="--", alpha=0.6)
        ax.legend()
        _save(fig, save_dir / f"{model_name.replace(' ', '_')}_rho_traj")
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="Koopformer-PRO benchmark (HPC-ready) with extended baselines and ρ_max ablation")
    p.add_argument("--file", type=str, default="./pressure_surface_2020.npy")
    p.add_argument("--epochs", type=int, default=4000)
    p.add_argument("--seq_len", type=int, default=120)  # for rho_ablation and grid
    p.add_argument("--save_dir", type=str, default="results/results_npj_baselines_pressure")
    p.add_argument("--mode", type=str, choices=["grid", "rho"], default="grid",
                   help="grid: full baseline sweep; rho: ρ_max ablation on full Koopformer variants")
    # rho-ablation-specific
    p.add_argument("--patch_len", type=int, default=120,
                   help="sequence/patch length for rho ablation")
    p.add_argument("--horizon", type=int, default=10,
                   help="forecast horizon for rho ablation")
    args = p.parse_args()
