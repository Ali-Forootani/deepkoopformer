...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 12 12:01:39 2025
...
# -*- coding: utf-8 -*-
"""
Koopformer-PRO benchmark on Wind-Speed data (HPC-ready, with train/test split)
...
# 3)  Positional encodings & patch embed (unchanged)
...
# 4) PatchTST backbone & Koopformer (unchanged)
...
def train(model, x_in, x_out, epochs=5000, lr=3e-4,
...
# 8) Main loop with train/test split and test evaluation
def main(file="bitstamp_windows.npy", epochs=200, seq_len=120,
    ...
    for patch_len in patch_lens:
        for horizon in horizons:
            ...
            # ---- Test data: use scaler from train ----
            if len(test_data) < (patch_len + horizon):
                print("Not enough data for test window, skipping this config.")
                continue
            ...
            pred_train, loss, eig = train(m, x_in_train, x_out_train,
                                    epochs=epochs, koop_attr="koop")
            res_plot["Koop-PatchTST"] = {"pred": pred_train, "loss": loss,
                                         "eig": eig, "x_out": x_np_train}
            ...
            # Plot test prediction for comparison
            plot_per_feature(x_np_test, {"PatchTST": pred_test},
...
if __name__ == "__main__":
    ...
    p.add_argument("--file", type=str, default="./crypto_coins.npy")
