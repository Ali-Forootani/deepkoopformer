...
# -*- coding: utf-8 -*-
"""
Created on Sun Nov 23 01:03:16 2025
...
Now extended with:
  ...
  - Saving test predictions & errors as in Code 2

2025-05-20  – Ali Forootani
Modified: extended baselines & ablations (2025-11-21, 2025-11-23)
"""
...
class StrictStableKoopmanOperatorGeneral(nn.Module):
    """
    ODO parameterisation with spectral radius < ρ_max.
    """
    def __init__(self, latent_dim: int, ρ_max: float = 0.99, act="sigmoid"):
        super().__init__()
    ...
    def squash(self, x):
        if self.act == "sigmoid":
            ...
            return torch.softmax(x, dim=0)  # sum = 1
        else:
            raise ValueError("Unknown squashing function")

    def forward(self, z):
        U, V = _orth(self.U_raw), _orth(self.V_raw)
        Σ = self.squash(self.S_raw) * self.ρ_max
...
class Koopformer_PatchTST(nn.Module):
    ...
    (ODO + Lyapunov; used for ablations (3) and (4) with
     different Lyapunov weights).
    """
    def __init__(self, input_dim, seq_len, horizon,
                 patch_len, d_model=64):
        super().__init__()
...
def build_dataset(data: np.ndarray, indices: list[int],
    ...
    Uses MinMaxScaler; if scaler is provided, it is reused (for test).
    """
    sub = data[:, indices]
    if scaler is None:
...
# --------------------------------------------------------------------------- #
# 12)  Saving (aligned with Code 2: Set column, test npy files)              #
# --------------------------------------------------------------------------- #
def save_results(model_name: str, set_name: str,
                 ...
                 save_npy: bool = False):
    """
    Save metrics row and optionally predictions/errors to disk.

    For consistency with Code 2:
      - metrics.csv has columns:
          Model, PatchLen, Horizon, Set, MSE, MAE
      - we typically save npy files only for the Test set.
    """
    err = x_out - pred
...
# --------------------------------------------------------------------------- #
# 13)  Main loop with Train/Test split                                       #
# --------------------------------------------------------------------------- #
def main(file="bitstamp_windows.npy", epochs=200, seq_len=120,
    ...
    # As in Code 2: include Set column
    ...
    for patch_len in patch_lens:
        for horizon in horizons:
            ...
            # Ensure enough samples for both train and test windows
            if len(train_data) < (patch_len + horizon) or \
               len(test_data)  < (patch_len + horizon):
            ...
            # --- Define all ablation/baseline variants --------------------- #
            model_variants = [
                ("DLinear",
                ...
                ("LSTM",
                 SimpleLSTMForecaster(F, hidden_dim=96,
                                      num_layers=2, horizon=horizon),
                 None, 0.0),

                ("PatchTST",
                 PatchTSTBaseline(F, patch_len, horizon,
                                  patch_len=patch_len, d_model=96),
                 None, 0.0),

                ("PatchTST+LinearLatent",
                 PatchTST_UnconstrainedLatent(F, patch_len, horizon,
                                              patch_len=patch_len,
                                              d_model=96),
                 None, 0.0),

                ("KoopPatchTST_NoLyap",
                 Koopformer_PatchTST(F, patch_len, horizon,
                                     patch_len=patch_len, d_model=96),
                 "koop", 0.0),
            ...
            for name, model, koop_attr, lyap_w in model_variants:
                print(f"== {name} ==")
                # ---- Train on train set ----------------------------------- #
                pred_train, loss, eig = train(
                    model, x_in_train, x_out_train,
                    epochs=epochs, koop_attr=koop_attr,
                    lyap_weight=lyap_w)

                # Save model (name and prefix consistent with previous script)
                prefix = save_dir / f"{name}_{patch_len}_{horizon}"
                torch.save(model.cpu().state_dict(), f"{prefix}.pt")

                # ---- Train metrics ---------------------------------------- #
                save_results(name, "Train", prefix,
                             x_np_train, pred_train,
                ...
                save_results(name, "Test", prefix,
                             x_np_test, pred_test,
                             metrics_file, patch_len, horizon,
                             save_npy=True)   # save *_test_predictions/errors

                # Store for plotting (train set)
                res_plot[name] = {
            ...
            # ----- Figures: training loss + spectra (train only) ----------- #
            plot_training(res_plot,
                          save_dir / f"train_patch{patch_len}_h{horizon}")

            # Per-feature forecast plots for train
            plot_per_feature(
                x_np_train,
            ...
            # Per-feature forecast plots for test (use test preds)
            plot_per_feature(
                x_np_test,
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="Koopformer-PRO benchmark (HPC-ready) with extended baselines and test eval")
